...
export function LessonPlanDetailPage() {
    ...
    const noticeMessage = useMemo(() => {
        ...
        if (notice === 'generated') {
            return mode === 'ai'
                ...
                : mode === 'fallback'
                    ? `Rule-based draft generated after AI fallback. ${referencesLabel}`
        ...
        if (notice === 'existing') {
            return mode === 'ai'
                ...
                : mode === 'fallback'
                    ? `Existing rule-based fallback draft opened. ${referencesLabel}`
    ...
    const generationMode = lessonPlan.generated_by_ai
        ? 'ai'
        : fallbackReason
            ? 'fallback'
            : 'rule-based';
    const generationLabel = generationMode === 'ai' ? 'AI draft' : 'Rule-based draft';
    const generationBadgeClassName = generationMode === 'ai'
        ? 'bg-purple-50 text-purple-700'
        : generationMode === 'fallback'
            ? 'bg-amber-50 text-amber-700'
            : 'bg-gray-100 text-gray-700';
    const generationSummary = generationMode === 'ai'
        ? 'Generated by AI'
        : generationMode === 'fallback'
                    ...
                    <div className="space-y-2">
                        ...
                        {fallbackReason ? (
                            <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                                <span className="font-medium">AI fallback used.</span>{' '}
                                {fallbackReason}
                            </div>
                        ) : null}
                    </div>
            ...
            <Card>
                <div className="grid grid-cols-1 gap-4 text-sm md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 md:col-span-2 xl:col-span-3">
                        ...
                        {fallbackReason ? (
                            <p className="mt-2 text-sm text-amber-800">Fallback reason: {fallbackReason}</p>
                        ) : null}
                    </div>
                </div>
            </Card>
