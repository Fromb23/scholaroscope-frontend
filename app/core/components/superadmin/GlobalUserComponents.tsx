...
interface RoleIconProps { role: string }
...
interface StatsBarProps { users: GlobalUser[] }
...
const EMPTY_CREATE: CreateFormData = {
    ...
    role: 'INSTRUCTOR', organization_id: '', phone: '',
    password: '', password2: '',
};
...
interface CreateUserModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: UserCreatePayload & { organization_id?: number }) => Promise<void>;
    submitting: boolean;
...
export function CreateUserModal({
    ...
    const validate = (): boolean => {
        ...
        if (!form.organization_id) e.organization_id = 'Organization is required';
...
export function EditUserModal({ isOpen, onClose, onSubmit, user, submitting }: EditUserModalProps) {
...
export function UsersTable({
    ...
    const columns: Column<GlobalUser>[] = [
        ...
        },
        {
            ...
            render: row => (
                <div className="flex items-center gap-1.5">
                    ...
                    <Badge
                        variant={ROLE_COLORS[(row.role ?? 'INSTRUCTOR') as UserRole]}
                        size="sm"
                    >
        ...
        {
            key: 'organization_name', header: 'Organization', sortable: true,
...
interface AddToOrgModalProps {
    ...
    organizations: { id: number; name: string }[];
    lockedOrgId?: number;       // pre-fills and locks org (used from org detail page)
    lockedOrgName?: string;
    submitting: boolean;
}

export function AddToOrgModal({
    isOpen, onClose, onSubmit, user, organizations,
    lockedOrgId, lockedOrgName, submitting,
}: AddToOrgModalProps) {
    const [orgId, setOrgId] = useState<string>(lockedOrgId ? String(lockedOrgId) : '');
    const [role, setRole] = useState('INSTRUCTOR');
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        setOrgId(lockedOrgId ? String(lockedOrgId) : '');
        setRole('INSTRUCTOR');
        setError(null);
    }, [isOpen, lockedOrgId]);
    ...
    const handleSubmit = async () => {
        ...
        setError(null);
        try {
            await onSubmit(user.id, Number(orgId), role);
            onClose();
        } catch (err: unknown) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Add ${user?.full_name ?? 'User'} to Organization`} size="sm">
            <div className="space-y-4">
                {error && <ErrorBanner message={error} onDismiss={() => setError(null)} />}

                {user && (
                    <div className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                        <p className="text-xs text-gray-500 font-medium uppercase tracking-wide mb-1">User</p>
