...
import {
    ...
    buildPendingAssessmentReviewHref,
...
export function BreakCleanupActions({ context, variant }: { context: TeachingTodayContext; variant: 'break' | 'exam' }) {
    const rows = getPendingSessionItems(context).slice(0, 4);
    const isExam = variant === 'exam';
    const pendingLessonHref = buildPendingLessonCleanupHref(context);
    const assignmentHref = buildAssignmentReviewHref(context);
    const assessmentHref = buildPendingAssessmentReviewHref(context);
    const hasActions = Boolean(pendingLessonHref || assignmentHref || assessmentHref);

    if (!hasActions && rows.length === 0) {
        return null;
    }
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2">
                    ...
                    {assignmentHref ? (
                        <ActionCard
                            ...
                            body="Open the assignment workspace for the pending learner responses."
                        />
                    ) : null}
                    {assessmentHref ? (
                        <ActionCard
                            ...
                            body="Open the assessment records that still have learner marks pending."
                        />
                    ) : null}
                </div>
            ) : null}
...
export function AfterBreakRecoverySuggestions({ context }: { context: TeachingTodayContext }) {
    ...
    const hasInsights = deriveMidtermInsights(context, 1).length > 0;

    if (!firstTeachingAssignment) {
        return null;
    }

    const resumeLabel = `Resume ${firstTeachingAssignment.cohort_name} ${firstTeachingAssignment.subject_name} from the next planned topic.`;
...
export function LastDayReadinessPanel({ context }: { context: TeachingTodayContext }) {
    ...
    const pendingCount = getPendingSessionItems(context).length;
    const pendingLessonHref = buildPendingLessonCleanupHref(context);
    const hasInsights = deriveMidtermInsights(context, 1).length > 0;
    ...
    return (
        <section className="theme-card rounded-2xl border theme-border p-4 sm:p-6" aria-labelledby="resume-soon">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                ...
                {pendingLessonHref ? (
                    <Link href={pendingLessonHref} className="teaching-today-nested-card rounded-xl p-4 transition-colors theme-hover-surface">
                        <p className="text-xs font-semibold uppercase theme-subtle">Pending records</p>
                        <p className="mt-1 text-sm font-semibold theme-text">
                            {pendingCount} can be closed before learning resumes.
                        </p>
...
export function AcademicBreakDashboard(props: AcademicBreakDashboardProps) {
    const { user } = useAuth();
    const hasInsights = deriveMidtermInsights(props.context, 1).length > 0;
    const hasBreakActions = Boolean(
        buildPendingLessonCleanupHref(props.context)
        || buildAssignmentReviewHref(props.context)
        || buildPendingAssessmentReviewHref(props.context)
        || getPendingSessionItems(props.context).length > 0
    );
    const hasRecoverySuggestions = props.context.teachingLoad.length > 0;
    const hasOperationalColumn = hasInsights || hasBreakActions || hasRecoverySuggestions;
    ...
    return (
        <div className="mx-auto max-w-7xl space-y-5 px-0 sm:space-y-6">
            ...
            <section className="grid gap-5 xl:grid-cols-12 xl:gap-6">
                ...
                <aside className={`space-y-5 ${hasOperationalColumn ? 'xl:col-span-5' : 'xl:col-span-12'}`}>
