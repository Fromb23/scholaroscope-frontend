...
export function ReportPoliciesHubPageClient() {
    ...
    return (
        <div className="space-y-6">
            ...
            {surfaces.length === 0 ? (
                <NoPolicySurfacesState />
            ) : (
                <div className="grid gap-4 lg:grid-cols-2">
                    {surfaces.map((surface) => (
                        <PolicySurfaceCard key={surface.key} surface={surface} />
                    ))}
                </div>
            )}
        </div>
...
function NoPolicySurfacesState() {
    return (
...
function PolicySurfaceCard({ surface }: { surface: PolicySurfaceDefinition }) {
    return (
        <Link href={surface.href} className="group">
