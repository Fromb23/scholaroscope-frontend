...
import { buildAssignmentDetailHref as buildCanonicalAssignmentDetailHref } from '@/app/core/lib/operationalDetailNavigation';
...
function normalizeQueryParam(value: string | null, options: Array<{ value: string }>): string {
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1 text-sm theme-muted">
            ...
            {curriculumName || curriculumType ? (
              <p>{[curriculumName, curriculumType].filter(Boolean).join(' · ')}</p>
            ) : null}
        ...
        <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
          {[
            ...
            { label: 'Issued', value: publishedCount, tone: 'theme-text' },
            {
              label: 'Due soon',
              value: dueSoonCount,
              tone: dueSoonCount > 0 ? 'text-[color:var(--color-warning)]' : 'theme-text',
            },
            {
              label: 'Overdue',
              value: overdueCount,
              tone: overdueCount > 0 ? 'text-[color:var(--color-danger)]' : 'theme-text',
            },
          ].map((metric) => (
            <div
              key={metric.label}
              className="rounded-lg border theme-border theme-surface-elevated px-3 py-3"
            >
...
export default function CohortAssignmentsPage() {
  ...
  const [evaluationTypeFilter, setEvaluationTypeFilter] = useState<AssignmentEvaluationType | ''>(
    normalizeQueryParam(searchParams.get('evaluation_type'), EVALUATION_OPTIONS) as
      | AssignmentEvaluationType
      | '',
  );
  const [deliveryModeFilter, setDeliveryModeFilter] = useState<AssignmentDeliveryMode | ''>(
    normalizeQueryParam(searchParams.get('delivery_mode'), DELIVERY_MODE_OPTIONS) as
      | AssignmentDeliveryMode
      | '',
  );
  const [reviewFilter, setReviewFilter] = useState(
    searchParams.get('review') === 'needs_review' ? 'needs_review' : '',
  );
  ...
  const newWorkUnavailable =
    !selectedTermAcceptsNewWork ||
    (newWorkEligibility?.allowed ?? todayMode?.allows_new_teaching ?? true) === false;
  const newWorkUnavailableReason =
    newWorkEligibility?.reason ??
    (!selectedTermAcceptsNewWork
      ? 'This academic term is read-only for new assignment work.'
      : null) ??
  ...
  const buildAssignmentsHref = useCallback(
    (nextCohortSubjectId?: string | null, options?: { includeHighlightAssignment?: boolean }) => {
  ...
  const buildAssignmentDetailHref = (nextAssignmentId: number) =>
    buildCanonicalAssignmentDetailHref(cohortId, nextAssignmentId, assignmentsHref);
  ...
  const showingWorkspaceSelection =
    !cohortSubjectFilter || (isTeachingActor && !selectedCohortSubject);
      ...
      [...visibleCohortSubjects]
        ...
        .map((subject) => {
          ...
          return {
            ...
            draftCount: subjectAssignments.filter((assignment) => assignment.status === 'DRAFT')
              .length,
            publishedCount: subjectAssignments.filter(
              (assignment) => assignment.status === 'PUBLISHED',
            ).length,
            dueSoonCount: subjectAssignments.filter((assignment) => isAssignmentDueSoon(assignment))
              .length,
            overdueCount: subjectAssignments.filter((assignment) => isAssignmentOverdue(assignment))
              .length,
          };
        }),
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm theme-muted">
                Cohort is the navigation context, and assignment work stays scoped to subject groups
                taught in this cohort.
              </p>
            </div>
      ...
      {pageError ? <ErrorBanner message={pageError} onDismiss={() => void refetch()} /> : null}

      {resultMessage ? (
        <div className="theme-success-surface rounded-lg px-4 py-3 text-sm">{resultMessage}</div>
      ) : null}

      {isTeachingActor && visibleCohortSubjects.length === 0 ? (
        <Card>
          <p className="text-sm theme-muted">No class subjects are assigned to you yet.</p>
        </Card>
      ) : null}
          ...
          <Card className="theme-surface-muted">
            <div className="space-y-2">
              ...
              <p className="text-sm theme-muted">
                Open the assignment workspace for a specific class subject before working through
                drafts, publishing, or review.
              </p>
              ...
              <div className="grid gap-4 lg:grid-cols-5">
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
                  <Select
                    ...
                    onChange={(event) =>
                      setDeliveryModeFilter(event.target.value as AssignmentDeliveryMode | '')
                    }
                  ...
                  <Select
                    ...
                    onChange={(event) =>
                      setEvaluationTypeFilter(event.target.value as AssignmentEvaluationType | '')
                    }
        ...
        <div className="space-y-6">
          <Card className="theme-border theme-surface-muted">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div className="space-y-2">
                ...
                <p className="text-sm theme-muted">
                  This workspace stays scoped to one class subject. Search and filters apply inside
                  this class only.
                </p>
                <p className="text-sm theme-muted">
          ...
          <StatStrip mdColumns={2} xlColumns={4}>
            <StatsCard
              ...
              color="blue"
            />
            <StatsCard title="Issued" value={publishedCount} icon={BookOpen} color="green" />
            <StatsCard
              title="Due Soon"
              value={dueSoonCount}
      ...
      <AssignmentCreateModal
        isOpen={createOpen && (editingAssignment ? canManageAssignments : canCreateAssignments)}
        onClose={() => {
          setCreateOpen(false);
          setEditingAssignment(null);
        }}
        ...
        defaultCohortSubjectId={
          selectedCohortSubject?.id ??
          (visibleCohortSubjects.length === 1 ? visibleCohortSubjects[0].id : null)
        }
