...
import { ArrowLeft, BookOpen, ClipboardList, Clock, FileCheck2, Users } from 'lucide-react';
...
function summarizeEvaluation(evaluation: AssignmentEvaluation, totalMarks: number | null): string {
...
export default function CohortAssignmentDetailPage() {
  ...
  const isInstitutionAdminView =
    activeOperatingContext === 'WORKSPACE_MANAGEMENT' && !isTeachingActor;
  const isValidRoute =
    Number.isFinite(cohortId) && cohortId > 0 && Number.isFinite(assignmentId) && assignmentId > 0;
  ...
  const allowed = !user
    ? false
    : isInstitutionAdminView || (isTeachingActor && instructorAccess.cohortIds.includes(cohortId));
  ...
  const { cohort, loading: cohortLoading } = useCohortDetail(isValidRoute ? cohortId : null);
  const { cohortSubjects, loading: cohortSubjectsLoading } = useCohortSubjects(
    isValidRoute ? cohortId : undefined,
  );
  const {
    assignment,
  ...
  const assignmentTitle = assignmentError
    ? String(assignmentError).toLowerCase().includes('denied')
      ? 'Access denied'
  ...
  const assignmentsHref = useMemo(
    () =>
      resolveOperationalDetailBack({
        ...
        hierarchicalParent: `/academic/cohorts/${cohortId}/assignments`,
        structuralFallback: '/assignments',
      }),
    [cohortId, searchParams],
  );
  const returnLabel = useMemo(
    () => getOperationalDetailBackLabel(assignmentsHref),
    [assignmentsHref],
  );
  const currentReturnTo = useMemo(() => {
    const query = searchParams.toString();
    const candidate = query ? `${pathname}?${query}` : pathname;
    return isSafeNextPath(candidate)
      ? candidate
      : `/academic/cohorts/${cohortId}/assignments/${assignmentId}`;
  ...
  const navigateWorkflowUnit = useCallback(
    (unit: string | null) => {
      if (!unit) return;
      if (!confirmDiscardWorkflowChanges()) return;
  ...
  const individualWorkUnits = useMemo(
    () =>
      [...recipientsQuery.recipients]
        ...
        .sort(
          (left, right) =>
            left.student_name.localeCompare(right.student_name) || left.student - right.student,
        ),
    [recipientsQuery.recipients],
  );
  const groupWorkUnits = useMemo(
    () =>
      [...groupsQuery.groups].sort(
  ...
  const evaluationBySubmissionId = useMemo(
    () =>
      new Map(
        evaluationsQuery.evaluations.map((evaluation) => [evaluation.submission, evaluation]),
      ),
  ...
  const reviewableSubmissions = useMemo(
    () =>
      [...submissionsQuery.submissions].sort(
        (left, right) =>
  ...
  const selectedRecordRecipient = useMemo(() => {
    ...
    if (selectedStudentIdFromUnit && Number.isFinite(selectedStudentIdFromUnit)) {
      const matched = individualWorkUnits.find(
        (recipient) => recipient.student === selectedStudentIdFromUnit,
      );
  ...
  const selectedReviewSubmission = useMemo(() => {
    if (isGroupAssignment) return null;
    if (selectedStudentIdFromUnit && Number.isFinite(selectedStudentIdFromUnit)) {
      const matched = reviewableSubmissions.find(
        (submission) => submission.student === selectedStudentIdFromUnit,
      );
      if (matched) return matched;
    }
  ...
  const recordRecipientIndex = selectedRecordRecipient
    ? individualWorkUnits.findIndex(
        (recipient) => recipient.student === selectedRecordRecipient.student,
      )
    : -1;
  ...
  const lifecycleSupplementaryActions =
    lessonPlanReturnHref && assignment?.status === 'DRAFT'
      ? [
          {
    ...
    () => ({
      ...
      state: {
        ...
        unfinished_work_count:
          ...
          lifecycleState.next_action !== 'NONE'
            ? 1
            : 0,
      ...
      workflowStep: lifecycleState
        ? `assignment_${getAssignmentCurrentProgressStage(lifecycleState).toLowerCase()}`
        : 'assignment_loading',
      emptyStateReason:
        !assignmentLoading && !assignment ? 'This assignment could not be loaded.' : undefined,
    }),
    [
      assignment,
    ...
    const selectedUnit =
      activeWorkflow === 'record'
        ? selectedRecordRecipient
          ? individualUnitValue(selectedRecordRecipient.student)
          : null
        : selectedReviewSubmission
          ? individualUnitValue(selectedReviewSubmission.student)
          : null;
    if (!selectedUnit) {
      updateWorkspaceUrl({ workflow: null, unit: null });
      return;
  ...
  const buildAssignmentLearnerReportHref = (learnerId: number) =>
    buildLearnerAssignmentReportHref(learnerId, {
  ...
  const openRecordResponsePanel = () => {
    if (isGroupAssignment) {
      updateWorkspaceUrl({
        workflow: 'record',
        unit: selectedGroup
          ? groupUnitValue(selectedGroup.id)
          : groupWorkUnits[0]
            ? groupUnitValue(groupWorkUnits[0].id)
            : null,
  ...
  const openReviewWorkflow = (submissionId?: number | null) => {
    if (isGroupAssignment) {
      const nextReviewGroup =
        groupWorkUnits.find(
          (group) => (group.submission_count ?? 0) > (group.evaluation_count ?? 0),
        ) ??
        selectedGroup ??
        groupWorkUnits[0] ??
        null;
  ...
  const handleApplyReviewToAllPending = async () => {
    if (isGroupAssignment) {
      setActionError(
        'Group bulk review needs the active group submission queue loaded before applying shared review values.',
      );
      return;
    }
    ...
    const narrative =
      window.prompt(`Shared feedback for ${pendingIds.length} pending response(s):`) ?? '';
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant={getAssignmentStatusBadgeVariant(assignment.status)}>
                  {lifecycleState?.teacher_stage_label ??
      ...
      {successMessage ? (
        <div className="theme-success-surface rounded-lg px-4 py-3 text-sm">{successMessage}</div>
      ) : null}
      ...
      <StatStrip mdColumns={2} xlColumns={4}>
        {isGroupAssignment ? (
          <>
            <StatsCard title="Groups" value={assignment.group_count} icon={Users} color="blue" />
            <StatsCard
              title="Submissions"
      ...
      {showSessionScopeNote ? (
        <Card className="theme-info-surface">
          <div className="space-y-1">
            ...
            <p className="text-sm theme-muted">
              This assignment can include learners from all active classes linked to the source
              session.
        ...
        <Card className="theme-info-surface">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge
                variant={assignment.attachment_policy === 'REQUIRED' ? 'orange' : 'blue'}
                size="sm"
              >
            ...
            {assignmentAttachmentSlots.length > 0 ? (
              <div className="grid gap-2 md:grid-cols-2">
                {assignmentAttachmentSlots.map((slot) => (
                  <div
                    key={slot.key}
                    className="rounded-lg border theme-border bg-white/70 px-3 py-2 text-sm"
                  >
          ...
          <Card className="theme-info-surface space-y-4">
            ...
            <div className="space-y-2 rounded-lg border theme-border bg-white/80 p-4">
              ...
              <p className="text-xs theme-muted">
                Submitted {formatDateTime(selectedReviewSubmission.submitted_at)} · Attachments{' '}
                {selectedReviewSubmission.attachment_metadata.length}
              </p>
            </div>
            ...
            <AssignmentReviewForm
              ...
              readOnly={Boolean(
                selectedReviewEvaluation?.evidence_created ||
                selectedReviewEvaluation?.evidence_record_id,
              )}
              onDirtyChange={setWorkflowDirty}
            />
          </Card>
      ...
      <ResponsiveActionSheet
        open={Boolean(
          canChangeActiveAssignment && activeWorkflow && !isGroupAssignment && useWorkflowSheet,
        )}
      ...
      <Card>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm theme-muted">
              Recipients, submissions, evaluations, groups, and evidence records are available when
            ...
            <div className="space-y-4">
              <Card>
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div className="space-y-1">
                    <div className="text-xs font-medium uppercase tracking-wide theme-muted">
                      Delivery
                    </div>
                    <div className="text-sm font-medium theme-text">
                      {getAssignmentDeliveryLabel(assignment.delivery_mode)}
                  ...
                  <div className="space-y-1">
                    <div className="text-xs font-medium uppercase tracking-wide theme-muted">
                      Learner Work Finished
                    </div>
                    <div className="text-sm font-medium theme-text">
                      {formatDateTime(assignment.closed_at)}
                    </div>
                  </div>
              ...
              <Card>
                <div className="space-y-4">
                  ...
                  {assignment.outcomes.length === 0 ? (
                    <p className="text-sm theme-muted">
                      No outcome metadata is attached to this assignment.
                    </p>
                  ) : (
                      ...
                      {assignment.outcomes.map((outcome) => (
                        <div key={outcome.id} className="rounded-lg border theme-border p-4">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {outcome.plugin ? (
                              <Badge
                                variant={outcome.plugin === 'cbc' ? 'green' : 'default'}
                                size="sm"
                              >
                                {outcome.plugin}
                              </Badge>
                        ...
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                          <div className="space-y-1">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge
                                variant={getAssignmentEvaluationBadgeVariant(
                              ...
                              evaluation.evidence_status === 'CREATED' ? (
                                <Badge variant="green" size="sm">
                                  Evidence created
                                </Badge>
      ...
      <AssignmentPublishModal
        ...
        onPublished={() => {
          setSuccessMessage(
            publishMode === 'issue' ? 'Assignment issued.' : 'Learners added to assignment.',
          );
