...
function buildSubjectMeta(subject: CbcAllowedSubject, defaultMeta: string): string {
    if (subject.blocked_reason) {
        return subject.blocked_reason;
    }

    if (subject.org_registration_status === 'NOT_REGISTERED_BY_ORG') {
        return 'Curriculum ready, not registered by organization';
    }
...
export function CBCCohortSubjectPanel({
    ...
    const handleLink = async (subject: CbcAllowedSubject) => {
        setWorking(true);
        setError(null);
        try {
    ...
    const profileSummary = snapshot.pathway
        ? `${snapshot.pathway.name}${snapshot.track ? ` · ${snapshot.track.name}` : ''}`
        : 'No CBC pathway configured yet.';
    const combinationSummary = snapshot.combination
        ? `Detailed mapping: combination #${snapshot.combination.official_code}${snapshot.combination.name ? ` · ${snapshot.combination.name}` : ''}`
        : null;
        ...
        <div className="space-y-4">
            ...
            <div className="rounded-xl border border-blue-100 bg-blue-50 p-4">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {snapshot.pathway ? <Badge variant="default">Configured</Badge> : <Badge variant="warning">Needs profile</Badge>}
                </div>
                <p className="mt-2 text-sm font-medium text-gray-900">{profileSummary}</p>
                {combinationSummary ? (
                    <p className="mt-1 text-xs text-gray-600">{combinationSummary}</p>
                ) : null}
                {!snapshot.pathway ? (
                    <p className="mt-1 text-xs text-gray-600">
                        Configure CBC pathway before linking pathway subjects. Core subjects remain visible, but pathway subject linking stays blocked until setup is complete.
                    </p>
                ) : null}
            </div>
                    ...
                    <div className="space-y-2">
                        {snapshot.core.map((subject) => (
                            <SubjectRow
                                ...
                                tone="core"
                                meta={buildSubjectMeta(subject, 'Core subject requirement')}
                            />
                        ))}
                    </div>
                )}
            ...
            <div className="space-y-2">
                ...
                {snapshot.pathway_subjects.length === 0 ? (
                    <p className="rounded-lg border border-dashed border-gray-200 bg-gray-50 px-3 py-4 text-center text-sm text-gray-500">
                        No pathway subjects are available until a cohort CBC pathway is configured.
                    </p>
                ) : (
                    <div className="space-y-2">
                        {snapshot.pathway_subjects.map((subject) => {
                            const linked = subject.subject_id !== null && linkedSubjectIds.has(subject.subject_id);
                            const canRegisterAndLink = (
                                !linked
                                && subject.subject_id === null
                                && subject.teachability_status === 'CURRICULUM_READY_NOT_REGISTERED_BY_ORG'
                            );
                            const canLink = linked || subject.is_teachable || canRegisterAndLink;
                            ...
                            return (
                                <SubjectRow
                                    ...
                                    readOnly={isHistorical}
                                    tone="pathway"
                                    meta={buildSubjectMeta(subject, 'Ministry pathway catalogue subject')}
                                    actionLabel={linked ? 'Remove' : canRegisterAndLink ? 'Register and Link' : 'Link'}
                                    actionDisabled={working || !canLink}
                                    actionTitle={!canLink ? subject.blocked_reason ?? undefined : undefined}
                                    onAction={() => (linked ? handleUnlink(subject.subject_id) : handleLink(subject))}
                                />
                            );
                        })}
                    </div>
                        ...
                        {snapshot.blocked.map((subject) => (
                            ...
                            >
                                <div className="min-w-0 space-y-1">
                                    ...
                                    <p className="text-xs text-gray-500">{subject.blocked_reason ?? subject.reason}</p>
                                </div>
