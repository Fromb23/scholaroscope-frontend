...
import { ArrowLeft, BookOpen, Calendar, Filter, Search, Target } from 'lucide-react';
...
export function CBCTeachingSessionsPage() {
  ...
  const filteredSessions = page.filtered;
  const groupedSessions = useMemo(() => {
    ...
    filteredSessions.forEach((session) => {
      const cohortName = session.cohort_name?.trim() || 'Unassigned class';
      const subjectName = session.subject_name?.trim();
      const existing = groups.get(cohortName);
      ...
      groups.set(cohortName, {
        cohortName,
        sessions: [session],
        subjects: new Set(subjectName ? [subjectName] : []),
      });
    });

    return Array.from(groups.values()).map((group) => {
      const subjectCount = group.subjects.size;
      const firstSubject = Array.from(group.subjects)[0];
  ...
  const primarySession = filteredSessions[0];
  const primarySessionId = primarySession?.id;
  const filteredCount = filteredSessions.length;
  const isEmpty = !page.isLoading && filteredCount === 0;
  const assistantContext = useMemo(
    () => ({
          ...
          ? [
              {
                ...
                type: 'navigate' as const,
                href: `/cbc/teaching/sessions/${primarySessionId}/outcomes`,
              },
            ]
          : []),
      ],
      nextSafeAction: primarySessionId
        ? {
            label: 'Open CBC Session',
            type: 'navigate' as const,
            href: `/cbc/teaching/sessions/${primarySessionId}/outcomes`,
      ...
      workflowStep: primarySessionId ? 'open_cbc_session' : 'cbc_session_filters',
      emptyStateReason: isEmpty ? 'No CBC sessions match the current filters.' : undefined,
    }),
    [filteredCount, isEmpty, page.isLoading, page.statusFilter, primarySessionId],
  );
  ...
  return (
    <div className="space-y-6">
      <CBCNav />
      <CBCBreadcrumb
        segments={[{ label: 'Teaching', href: '/cbc/teaching' }, { label: 'All Lessons' }]}
      />
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {filteredSessions.length} lesson{filteredSessions.length !== 1 ? 's' : ''} found
              {groupedSessions.length > 0
                ? ` in ${groupedSessions.length} class${groupedSessions.length !== 1 ? 'es' : ''}`
                : ''}
            </p>
          </div>
              ...
              >
                <div className="flex flex-col gap-3 border-b border-gray-100 bg-gray-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    <h3 className="truncate font-semibold text-gray-900">{group.cohortName}</h3>
                    <p className="mt-0.5 text-sm text-gray-500">
                      {group.sessions.length} lesson{group.sessions.length !== 1 ? 's' : ''}
                      {group.subjectSummary ? ` • ${group.subjectSummary}` : ''}
                    </p>
                  </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {(session.outcome_links_count ?? 0) > 0 && (
                              <span className="text-xs text-gray-500 flex items-center gap-1">
                                ...
                                {session.outcome_links_count} learning goal
                                {session.outcome_links_count !== 1 ? 's' : ''}
                              </span>
                            )}
                          </div>
                        </div>
