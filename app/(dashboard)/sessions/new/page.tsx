...
import { ArrowLeft, Save, Calendar, Clock, BookOpen, Users, AlertCircle, X } from 'lucide-react';
import Link from 'next/link';
import { Card } from '@/app/components/ui/Card';
import { Button } from '@/app/components/ui/Button';
import { Input } from '@/app/components/ui/Input';
import { Select } from '@/app/components/ui/Select';
import { TopicSubtopicPicker } from '@/app/core/components/sessions/TopicSubtopicPicker';
...
import { topicSessionLinkAPI } from '@/app/core/api/topics';
...
export default function CreateSessionPage() {
  ...
  const handleChange = (field: keyof SessionFormData, value: SessionFormData[keyof SessionFormData]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isCBC && selectedSubtopics.length > 0) {
        await Promise.all(
          selectedSubtopics.map(s =>
            topicSessionLinkAPI.create({ session: newSession.id, subtopic: s.id })
          )
        );
      }
      ...
      setSubmitError(extractErrorMessage(err as import('@/app/core/types/errors').ApiError));
          ...
          <Card>
            <div className="p-6">
              ...
              <p className="text-sm text-gray-500 mb-5">
                Select what you&apos;ll be teaching. Links this session to the curriculum and pre-fills the session title.
              </p>
              <TopicSubtopicPicker
                subjectId={subjectId}
